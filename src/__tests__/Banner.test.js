...
test('it shows the title in the banner', () => {
	setup();
	// We expect that the title 'Hi, Iam Revan' is in the banner component
	expect(screen.getByText(/Hi, Iam Revan/i)).toBeInTheDocument();
});
...
test('can download cv when clicked on download cv button', async () => {
	...
	// expect(downloadLink.download).toEqual('MochRevanoBudiansyah_CV.pdf');
	// expect(downloadLink.href).toEqual('/files/MochRevanoBudiansyah_CV.pdf');
